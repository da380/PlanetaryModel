...
concept HasNormalisationInformation = requires(T t) {
  ...
  { t.GravitationalConstant() } -> std::convertible_to<typename T::value_type>;
};
...
concept SphericalGeometryModel = requires(Model model, Model::size_type i,
  ...
  // Member functions returning the lower and upper radius of the ith layer.
  { model.LowerRadius(i) } -> std::convertible_to<typename Model::value_type>;
  { model.UpperRadius(i) } -> std::convertible_to<typename Model::value_type>;
...
concept SphericalElasticModel = requires(Model model, Model::size_type i,
  ...
  { model.Kappa(i) } -> std::regular_invocable<typename Model::value_type>;
  { model.Kappa(i)(r) } -> std::convertible_to<typename Model::value_type>;
